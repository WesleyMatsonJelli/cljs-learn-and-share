...
* [`src/cljs/cljs_learn_and_share/`](src/cljs/cljs_learn_and_share/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/cljs_learn_and_share/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `cljs-learn-and-share` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_learn_and_share/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-learn-and-share.example
  (:require [cljs-learn-and-share.config :as config])
